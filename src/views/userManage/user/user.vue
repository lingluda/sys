...
<template>
    <div>
        <Row>
           <Input v-model="comNameKeyWord" placeholder="登录账号" style="width: 160px"></Input>
            ...
            <Modal v-model="mod.add" title="新增" width="460">
                <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120">
                    ...
                    <FormItem label="手机号：" prop="phone" :maxlength="11">
            ...
            <Modal v-model="mod.edit" title="编辑" width="460">
                <Form ref="formEdit" :model="formEdit" :rules="ruleEditdate" :label-width="120">
                    ...
                    <FormItem label="手机号：" prop="phone" :maxlength="11">
                        <Input v-model="formEdit.phone"></Input>
                    </FormItem>
                    <FormItem label="分配角色：">
                    ...
                    <FormItem label="状态">
                        <Select v-model="status">
                            <Option v-for="item in zt" :value="item.value" :key="item.value">{{ item.label }}</Option>
                        </Select>
                    </FormItem>
                </Form>
                <div slot="footer">
            ...
            <Modal v-model="mod.role" title="配置角色" width="380">
                <Form :label-width="86">
                    <FormItem label="分配角色：">
                        <Select v-model="selRole">
                            <Option v-for="item in roles" :value="item.id" :key="item.id" selected="selected">{{ item.name }}</Option>
                        </Select>
                    </FormItem>
                </Form>
                <div slot="footer">
...
<script>
    ...
    export default {
            ...
            return {
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        key: "status",
                        render: (h, params) => {
                            return h('div', ['启用', '禁用'][params.row.status]);
                        }
                    },
                    {
                        title: "操作",
                                    ...
                                    "Button",
                                    {
                                        ...
                                        on: {
                                            click: () => {
                                                ...
                                                this.mod.edit = true;
                                                this.selRole = parseInt(params.row.roleId);
        ...
        methods: {
            getData() {
                let self = this, param = {
                    account: util.trim(self.comNameKeyWord),
                    pageIndex: this.page, //页码
                    pageSize: this.size, //每页条数
                }
                util.ajaxY("/sys/sysUser/list", param, function (d) {
                    if (d.code == 0) {
            ...
            deleteFn() {
                let self = this
                util.ajaxY('/sys/sysUser/del', {id: this.curId}, function (d) {
                    console.log(d.code);
                    if (d.code == 0) {
