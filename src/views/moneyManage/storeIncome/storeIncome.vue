...
<Modal v-model="mod.payDet" title="款项明细" width="1100">
    ...
    <div slot="footer">
        ...
        <Row class="tab">
            <RadioGroup v-model="sta" type="button" class="padding-left-20 margin-top-10 margin-bottom-10" size="large" @on-change="changeSta">
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      util.ajaxY("order.ShopsEarningsOrder",param,function(d) {
          if (d.code == 0) {
    ...
    payMoney(){
      ...
      let merid=this.selData[0].merid,total=0,oids=[],commis_rate=0;
      for (var i=0;i<len;i++){
        if(self.selData[i].merid!=merid){
          this.$Message.error("请选择同一个门店的订单进行打款");
          return false;
        }else{
    ...
    contrPayfn(){
      let self = this,
        param = {
          remitType:0,  //类型（0：门店 1：运营商）
          shId:this.payData.merid+'',  //门店/运营商id拼接字符串
          orderId:this.payData.oids,  //订单id拼接字符串
          objectid:this.payData.merid+'',  //门店id或运营商id
