        ...
        while (i < len) {
            if (i === -1) {
                ...
                tableInnerHTML += '<tr>';
                    ...
                    // 为td添加样式，避免显示成科学计数法, 以下是对应的显示格式，如果需要成对应的方式添加特殊的style
                ...
                Array.from(tableBody.rows[i].children).forEach((td) => {
                    tableInnerHTML = tableInnerHTML + '<td style="vnd.ms-excel.numberformat:#,##0.00">' + td.children[0].children[0].innerHTML + '</td>';
                });
                tableInnerHTML += '</tr>';
            }
            i++;
